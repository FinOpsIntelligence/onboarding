...
#   5. Assigning the "Savings plan reader" permission on the tenant-level
#      Microsoft.BillingBenefits scope.
#   6. Assigning the "Reservations Reader" permission on the tenant-level
#      Microsoft.Capacity scope.
#   7. Generating a JSON block with the credentials to be pasted into the Portal.
# ==============================================================================
...
# 4) Get Service Principal Object ID for tenant-level role assignments
echo "Fetching Service Principal Object ID..."
SP_OBJECT_ID=$(az ad sp show --id "$CLIENT_ID" --query id -o tsv 2>/dev/null || echo "")

if [ -n "$SP_OBJECT_ID" ]; then
  echo -e "  \e[1;32m✓ Service Principal Object ID retrieved successfully.\e[0m"
else
  echo -e "  \e[1;33m⚠️ Warning: Could not retrieve Service Principal Object ID.\e[0m"
  echo "  Tenant-level Savings Plan and Reservations roles could not be assigned automatically."
fi

# 5) Assign the Savings plan reader permission
echo "Assigning 'Savings plan reader' role on the tenant-level BillingBenefits scope..."
if [ -n "$SP_OBJECT_ID" ]; then
  if az role assignment create \
    ...
    echo -e "  \e[1;32m✓ 'Savings plan reader' role assigned successfully.\e[0m"
  else
    echo -e "  \e[1;33m⚠️ Warning: Failed to assign 'Savings plan reader' role.\e[0m"
    echo "  Your user account may not have permission to assign roles at the tenant-level BillingBenefits scope."
    echo "  You can assign this role manually after onboarding if needed."
  fi
else
  echo -e "  \e[1;33m⚠️ Warning: Skipping 'Savings plan reader' assignment because the Service Principal Object ID is unavailable.\e[0m"
fi

# 6) Assign the Reservations Reader permission
echo "Assigning 'Reservations Reader' role on the tenant-level Capacity scope..."
if [ -n "$SP_OBJECT_ID" ]; then
  if az role assignment create \
    --assignee-object-id "$SP_OBJECT_ID" \
    --assignee-principal-type "ServicePrincipal" \
    ...
    --output none 2>/dev/null; then
    echo -e "  \e[1;32m✓ 'Reservations Reader' role assigned successfully.\e[0m"
  else
    echo -e "  \e[1;33m⚠️ Warning: Failed to assign 'Reservations Reader' role.\e[0m"
    echo "  Your user account may not have permission to assign roles at the tenant-level Capacity scope."
    echo "  You can assign this role manually after onboarding if needed."
  fi
else
  echo -e "  \e[1;33m⚠️ Warning: Skipping 'Reservations Reader' assignment because the Service Principal Object ID is unavailable.\e[0m"
fi

# 7) Output the final JSON credentials block
echo ""
